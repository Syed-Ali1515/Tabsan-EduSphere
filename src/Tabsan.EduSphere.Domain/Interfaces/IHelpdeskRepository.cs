...
public interface IHelpdeskRepository
{
    ...
    Task<IReadOnlyList<SupportTicket>> GetTicketsBySubmitterAsync(Guid submitterId, CancellationToken ct = default);

    /// <summary>Returns tickets within the given department(s). Null = all departments (SuperAdmin).</summary>
    Task<IReadOnlyList<SupportTicket>> GetTicketsByDepartmentAsync(IReadOnlyList<Guid>? departmentIds, TicketStatus? status, CancellationToken ct = default);

    /// <summary>Returns tickets assigned to the given staff member.</summary>
    Task<IReadOnlyList<SupportTicket>> GetTicketsByAssigneeAsync(Guid assignedToId, CancellationToken ct = default);
