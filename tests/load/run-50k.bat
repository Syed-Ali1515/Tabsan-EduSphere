...
:run
echo Running 50K test against %BASE_URL%
if /I "%RAW_MODE%"=="raw" (
	"%K6_EXE%" run --summary-export "%SUMMARY_JSON%" --out "json=%RAW_JSON%" -e BASE_URL=%BASE_URL% -e LOCAL_VU_CAP=%LOCAL_VU_CAP% -e SUMMARY_TXT_PATH="%SUMMARY_TXT%" "%SCRIPT_DIR%k6-scale-50k.js"
) else (
	"%K6_EXE%" run --summary-export "%SUMMARY_JSON%" -e BASE_URL=%BASE_URL% -e LOCAL_VU_CAP=%LOCAL_VU_CAP% -e SUMMARY_TXT_PATH="%SUMMARY_TXT%" "%SCRIPT_DIR%k6-scale-50k.js"
)
