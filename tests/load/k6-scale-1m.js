...
const TARGET_RPS = Math.max(1, Number.parseInt(__ENV.TARGET_RPS || '900', 10));
...
const THINK_TIME_MIN = Number.parseFloat(__ENV.THINK_TIME_MIN || '0.20');
const THINK_TIME_MAX = Number.parseFloat(__ENV.THINK_TIME_MAX || '1.10');
...
export const options = {
  ...
  scenarios: {
    scale_1m: {
      ...
      stages: [
        { duration: '8m', target: Math.max(1, Math.floor(SHARD_TARGET_RPS * 0.2)) },
        { duration: '16m', target: Math.max(1, Math.floor(SHARD_TARGET_RPS * 0.6)) },
