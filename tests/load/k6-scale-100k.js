...
const TARGET_RPS = Math.max(1, Number.parseInt(__ENV.TARGET_RPS || '220', 10));
...
export const options = {
  ...
  scenarios: {
    scale_100k: {
      ...
      stages: [
        { duration: '4m', target: Math.max(1, Math.floor(SHARD_TARGET_RPS * 0.2)) },
        { duration: '10m', target: Math.max(1, Math.floor(SHARD_TARGET_RPS * 0.6)) },
        { duration: '20m', target: SHARD_TARGET_RPS },
  ...
  thresholds: {
    api_errors: ['rate<0.12'],
    api_duration: ['p(95)<4000'],
  },
};
...
export function setup() {
  ...
  const loginRes = http.post(
    `${BASE_URL}${LOGIN_PATH}`,
    JSON.stringify({ username: TEST_USERNAME, password: TEST_PASSWORD, deviceInfo: 'k6-scale-100k' }),
