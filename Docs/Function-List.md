...
| `Phase 32.3 active recipient phone-number resolution` | Resolves active user phone numbers for notification recipient sets in repository layer for SMS fan-out (placeholder: returns empty until User.PhoneNumber field is added). | `src/Tabsan.EduSphere.Domain/Interfaces/INotificationRepository.cs`, `src/Tabsan.EduSphere.Infrastructure/Repositories/NotificationAttendanceRepositories.cs` |
